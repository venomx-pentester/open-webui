<script lang="ts">
	...
	$: {
		const allShortcuts = Object.values(shortcuts).filter(
			(shortcut): shortcut is ShortcutDefinition => {
				if (!shortcut) {
					return false;
				}
				if (!shortcut.setting) {
				...
				return (
					($settings as Record<string, unknown>)[shortcut.setting.id] === shortcut.setting.value
				);
			}
		);
...
<Modal bind:show ariaLabel={$i18n.t('Keyboard Shortcuts')}>
	<div class="text-gray-700 dark:text-gray-100 px-5 py-4">
		<div class="flex justify-between dark:text-gray-300 pb-2">
			<div class="text-lg font-medium self-center" id="keyboard-shortcuts-title">
		...
		{#each Object.entries(categorizedShortcuts) as [category, items], categoryIndex}
			...
			<section
				aria-labelledby={`keyboard-shortcuts-${category.replaceAll(' ', '-').toLowerCase()}`}
				class="contents"
			>
				<div class="flex justify-between dark:text-gray-300 pb-2">
					<div
						class="text-base self-center"
						id={`keyboard-shortcuts-${category.replaceAll(' ', '-').toLowerCase()}`}
					>
						{$i18n.t(category)}
					</div>
				</div>
				<div class="flex flex-col md:flex-row w-full md:space-x-2 dark:text-gray-200">
					<div class="flex flex-col w-full sm:flex-row sm:justify-center sm:space-x-6">
						<div class=" grid grid-cols-1 sm:grid-cols-2 gap-2 gap-x-4 w-full" role="list">
							<!-- {$i18n.t('Chat')} -->
							<!-- {$i18n.t('Global')} -->
							<!-- {$i18n.t('Input')} -->
							<!-- {$i18n.t('Message')} -->
							...
							<!-- {$i18n.t('When "Ctrl+Enter to Send" is enabled, use Ctrl+Enter instead.')} -->
							<!-- {$i18n.t('When "Ctrl+Enter to Send" is enabled, Enter inserts a newline.')} -->
							...
							{#each items as shortcut}
								<div class="col-span-1 flex items-start" role="listitem">
